...
export const addProductToCart = async (cartId, productId, quantity) => {
    ...
    if (existingProduct.length > 0) {
        ...
        if (productStock[0].stock < newQuantity) {
            return { success: false, message: "Not enough stock available to update quantity." };
        }
...
export const getCartProducts= async(cart_id)=>{
    try {
        ...
        const { rows: cart } = await pool.query(
            `SELECT c.*,p.name,p.category_id, FROM CartProducts c JOIN Products p ON c.product_id=p.product_id WHERE c.cart_id = $1`,
            [cart_id]
        );
        if(cart.length == 0) {
